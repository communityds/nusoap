...
class wsdl extends nusoap_base
{
    ...
    /**
     * @var array Added to HTTP request headers
     */
    protected $outgoing_http_headers = [];

    /**
     * @var array array of wsdl docs to import
     */
    public $import = [];
            ...
            foreach ($this->messages as $msgName => $msgParts) {
                ...
                if (is_array($msgParts)) {
                    foreach ($msgParts as $partName => $partType) {
                        ...
                        if ($typeDef && $typeDef['typeClass'] == 'element') {
    ...
    protected function serializeType($name, $type, $value, $use = 'encoded', $encodingStyle = false, $unqualified = false)
    {
        ...
        if ($phpType == 'struct') {
            ...
            if (is_array($value) && sizeof($value) >= 1) {
                ...
                $contents = '';
                foreach ($value as $k => $v) {
                    $this->debug("serializing array element: $k, " . (is_array($v) ? var_export($v, true) : $v) . " of type: $typeDef[arrayType]");
